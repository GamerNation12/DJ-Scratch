...
async def apply_features(session, artist, song):
    import re
    print(f"[DEBUG apply_features] Called with artist: '{artist}', song: '{song}'")
    m = re.search(r"[\(\[](?:feat\.?|ft\.?|featuring)\s+([^\]\)]+)[\)\]]", song, flags=re.IGNORECASE)
    if m:
        features = m.group(1).strip()
        song = song.replace(m.group(0), "").strip()
        print(f"[DEBUG apply_features] Extracted from original song -> features: '{features}', new song: '{song}'")
        ...
        async with session.get(url) as r:
            if r.status == 200:
                ...
                if data.get('resultCount', 0) > 0:
                    ...
                    print(f"[DEBUG apply_features] iTunes returned it_artist: '{it_artist}', it_track: '{it_track}'")
                    
                    m2 = re.search(r"[\(\[](?:feat\.?|ft\.?|featuring)\s+([^\]\)]+)[\)\]]", it_track, flags=re.IGNORECASE)
                    if m2:
                        features = m2.group(1).strip()
                        print(f"[DEBUG apply_features] Extracted from it_track -> features: '{features}'")
                        return f"{artist}, {features}", song
                        ...
                        print(f"[DEBUG apply_features] Using it_artist: '{it_artist}' because it contains multiple artists")
                        return it_artist, song
                    else:
                        print(f"[DEBUG apply_features] No feature condition met for it_artist: '{it_artist}' vs original: '{artist}'")
                else:
                    print(f"[DEBUG apply_features] iTunes resultCount was 0.")
            else:
                print(f"[DEBUG apply_features] iTunes API returned status {r.status}")
    except Exception as e:
        print(f"[DEBUG apply_features] Exception: {e}")
        pass
        
    print(f"[DEBUG apply_features] Returning original artist: '{artist}', song: '{song}'")
    return artist, song
