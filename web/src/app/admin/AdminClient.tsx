...
export default function AdminClient() {
  ...
  useEffect(() => {
    if (session) {
      fetchApi("/api/admin/check").then(res => res.json()).then(data => {
        setRole(data.role);
        if (data.role) {
          fetchStats();
  ...
  const getStatusBadge = (s: string) => {
    switch(s) {
      ...
      case 'completed': return <span className="px-2 py-0.5 bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 rounded text-[10px] font-bold uppercase">Released</span>;
      default: return <span className="px-2 py-0.5 bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 rounded text-[10px] font-bold uppercase">Pending</span>;
    }
  };
